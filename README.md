...
**사이트 주소** : Kanatales-elastic-server-env.eba-imx4hypj.ap-northeast-2.elasticbeanstalk.com  
(현재 로드밸런서 프리티어사용량을 다써서 임시로 로드밸런서를 빼고 고정ip주소도 일단 없애고 배포했습니다.9월부터 다시 로드밸런서 붙여서 배포가능.문제는 고정IP를 NLB로 했던건데 고정IP를 적용하는것을 다른방식으로 가능한지 찾아봐야할듯.그리고 배포해둔것을 제거하고 다시 aws환경을 만드는과정에서 데이터를 백업안해놓고 다시해서 가챠시뮬레이터 데이터가 다날라갔습니다.데이터가 있을때 기능들은 밑에 확인은 가능합니다.참고부탁드립니다.**빠른시일내에 복구예정**)  
...
## 공유 덱 리스트
![sharedCommentss](https://github.com/jungtaemin/kanatales_deckmaker/assets/96284736/e5df7814-c8c4-4639-afeb-af45959b3c8a)
* 공유 덱 등록 기능 - 글제목,글내용을 입력할 수 있고 해당 계정에 등록된 덱 리스트가 출력되서 클릭시 해당 덱정보가 게시글에 등록된다.(비로그인시 등록버튼 안보임)
* 공유 덱 게시글에 댓글 등록 기능,댓글 갯수 체크
* 공유 덱 리스트 조회수 체크
* 공유 덱 이름으로 리스트 검색 기능
* 추가될 기능) 대댓글,내글 삭제
## 가챠 시뮬레이터
...
# 패치노트 v1.8(2023-08-21)
### 공유 덱 리스트
* 게시글 댓글 등록시 reload안되는문제 수정
* 게시글 전체 댓글수 카운팅 추가
* 공유 덱 등록시 덱목록 세로로 출력되는것 수정
* 게시글 커서모양 변경
* 미로그인시 댓글등록버튼클릭하면 오류 - 미로그인시 등록버튼클릭하면 로그인 하시겠습니까? ->로그인 수정
* 게시글 등록버튼 로그인시만 보였는데 댓글과같이 미로그인시 얼럿뜨는것으로 수정
