...
(현재 로드밸런서 프리티어사용량을 다써서 임시로 로드밸런서를 빼고 고정ip주소도 일단 없애고 배포했습니다.9월부터 다시 로드밸런서 붙여서 배포가능.문제는 고정IP를 NLB로 했던건데 고정IP를 적용하는것을 다른방식으로 가능한지 찾아봐야할듯.)  
...
* aws 로드밸런서를 빼고 다시 aws환경을 구축하는 과정에서 가챠시뮬레이터 데이터를 다날려서 50% 복구함(나머지 내일안에 모두 복구예정)
